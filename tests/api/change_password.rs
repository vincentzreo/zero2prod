...
async fn current_password_must_be_valid() {
    ...
    // Act - Part 3 - Follow the redirect
    let html_page = app.get_change_password_html().await;
    assert!(html_page.contains("<p><i>The current password you provided is incorrect.</i></p>"));
}

#[tokio::test]
async fn new_password_should_long_enough() {
    let app = spawn_app().await;
    // generate a less than 12-character long password
    let short_password = "a1a2a3a4";
    ...
    // Act - Part 3 - Follow the redirect
    let html_page = app.get_change_password_html().await;
    assert!(html_page.contains("<p><i>New password must be at least 12 characters long.</i></p>"));
}
