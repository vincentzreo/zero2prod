...
pub async fn subscribe(
    ...
    store_token(&mut transaction, subscription_id, &subscription_token)
        .await
        .context("Failed to store a new token in the database")?;
    transaction
        .commit()
        .await
        .context("Failed to commit SQL transaction to store a new token")?;
...
#[tracing::instrument(
    name = "Saving the subscription token in the database",
...
impl std::fmt::Display for StoreTOkenError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "A database error occurred while storing a new token: {}",
            self.0
        )
    }
}
